...
//		}
    ...
    public function get_post_limit()
    {
        ...
        if ($this->get_loading_mode() !== 'none') {
            if ($this->is_current_query()) {
                $post_limit = $this->get_settings('archive_posts_per_page') ?: get_option('archive_posts_per_page');
            } else {
    ...
    public function get_paged()
    {
        ...
        if (in_array($loading_mode, ['standard', 'ajax_pagination'], true)) {
            return max(the7_get_paged_var(), $this->get_val($_GET, AJAX_Pagination::WIDGET_PAGE_ID_PARAM . $this->widget->get_id()));
        }
